...
const help_lines = [_][]const u8{
    ...
    "  H, shift+left    scroll the focused panel left (long rows)",
    "  L, shift+right   scroll the focused panel right (long rows)",
    ...
    "  d              discard menu",
    "  D              discard all",
    "  s              stash menu (all, +untracked, staged, file, keep-changes);",
    "                 asks for an optional message (empty = git's default name)",
    "  /              filter by path",
    ...
    "Staging view",
    "  j, k           move by line",
    "  J, shift+down  jump to the next hunk",
    "  K, shift+up    jump to the previous hunk",
    "  v              toggle range selection",
    "  space          stage/unstage the line(s); on a @@ header the whole hunk",
    "  d              discard the line(s)/hunk (same selection as space); on the",
    "                 staged side it drops the change from the index too",
...
fn footerHints(c: FooterCtx) []const u8 {
    ...
    if (c.staging) {
        return "j/k line  J/K hunk  v range  space stage/unstage  d discard (@@=hunk)  [/] staged/unstaged  \\ split  c/A commit/amend  e edit  esc back" ++ global;
    }
