...
- **Small and fast.** A 1.9 MB static binary against 17.6 MB, one linked
  ...
  20 ms, 16 git subprocesses to load a repo against 25, 8 MB resident once
  settled against 35 MB, and about half the total CPU over a ten second
  window (an eighth of it in its own process), measured on the same machine
  and repository. Eleven panel loaders fan out in parallel, the log loads
  incrementally, previews are cached, and loading never touches the network.
  Ziggity does peak higher during load, 55 MB against a flat 35 MB, before
...
- **Nothing blocks, ever.** Fetch, pull, push and slow multistep actions run
  ...
  [Details](comparison.md#nothing-blocks-ever)
...
- **Mouse text selection with automatic copy.** Character precise click and
  ...
  [Details](comparison.md#copy-text-straight-from-the-screen)
...
- **The mouse works everywhere.** Click to focus a panel, click to select a
  ...
  graph.
  [Details](comparison.md#the-mouse-works-everywhere)
...
- **At home in any terminal.** Standard ANSI plus a curated 256 color set,
  ...
  [Details](comparison.md#at-home-in-any-terminal)
...
- **Checkout by name that lands on a real branch.** In the `c` prompt a
  ...
  meaning.
  [Details](comparison.md#checkout-by-name-that-lands-on-a-real-branch)
...
- **A commit editor that nudges the 50/72 rule.** A live subject counter
  ...
  configurable.
  [Details](comparison.md#a-commit-editor-that-nudges-the-5072-rule)
...
- **An escalating force push chain with no dead ends.** Rejected push, then
  ...
  [Details](comparison.md#a-force-push-that-never-dead-ends)
...
- **A diffing mode you can read off the screen.** One `W` marks the selected
  ...
  [Details](comparison.md#diffs-the-way-review-tools-show-them)
...
- **Word-level diff highlighting.** Within a changed `-`/`+` line pair, the
  ...
  [Details](comparison.md#word-level-diff-highlighting)
...
- **Soft-wrap long lines in every diff panel.** `ctrl+w` wraps long diff lines
  ...
  [Details](comparison.md#wrap-long-lines-on-demand)
...
- **Independent Branches and Commits drill downs.** Both panels keep their
  ...
  [Details](comparison.md#two-histories-side-by-side)
...
- **Small courtesies.** One remote skips the remote prompt on tag push. Any
  ...
  [Details](comparison.md#small-courtesies)
